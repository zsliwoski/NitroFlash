...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] NetworkGamemodeObjectNetworkObject = null;
		public GameObject[] NetworkPlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is NetworkGamemodeObjectNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkGamemodeObjectNetworkObject.Length > 0 && NetworkGamemodeObjectNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetworkGamemodeObjectNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
			...
			else if (obj is NetworkPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkPlayerNetworkObject.Length > 0 && NetworkPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetworkPlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateNetworkGamemodeObject instead, its shorter and easier to type out ;)")]
		public NetworkGamemodeObjectBehavior InstantiateNetworkGamemodeObjectNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkGamemodeObjectNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkGamemodeObjectBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetworkGamemodeObjectBehavior>().networkObject = (NetworkGamemodeObjectNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateNetworkPlayer instead, its shorter and easier to type out ;)")]
		public NetworkPlayerBehavior InstantiateNetworkPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkPlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetworkPlayerBehavior>().networkObject = (NetworkPlayerNetworkObject)obj;
		...
		public NetworkGamemodeObjectBehavior InstantiateNetworkGamemodeObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkGamemodeObjectNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkGamemodeObjectBehavior>();
		...
		public NetworkPlayerBehavior InstantiateNetworkPlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkPlayerBehavior>();
